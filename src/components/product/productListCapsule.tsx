...
const ProductCapsule = (props:any) => { // properties  
    ...
    return (
        <>
                ...
                <td className="px-4 py-3 items-center"><Image className="rounded-xl"  src={product.thumbnail} width="50" height="50" alt={product.productName}/></td>
                <td className="px-4 py-3">{product.id}</td>
                <td className="px-4 py-3">{product.productName}</td>
                <td className="px-4 py-3">{product.cateGet?.cateName}</td>
                <td className="px-4 py-3 max-w-[12rem] truncate">{product.title}</td>                         
                <td className="px-4 py-3 flex justify-center">
                    ...
                    <div id={product.name} style={{display:actprod?"block":"none"}} className="none right-2 absolute z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                        <ul className="py-1 text-sm" aria-labelledby={product.name}>
                            <li>
                                <Link href={`/home/product/${product.id}`} type="button" data-modal-target="readProductModal" data-modal-add="readProductModal" className="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
                ...
                <div className="inset-35 relative p-4 w-full max-w-md max-h-full">
                    ...
                    <div className="relative p-4 text-center bg-white border-solid border rounded-lg shadow dark:border-gray-600 dark:bg-gray-800 sm:p-5">
                        ...
                        <div className="flex justify-center items-center space-x-4">
                            ...
                            <button onClick={()=>{setDelprod(!delprod),handleDetele()}} className="py-2 px-3 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-500 dark:hover:bg-red-600 dark:focus:ring-red-900">Yes, I&apos;m sure</button>
                        </div>
                    </div>
                </div>
            </div>
